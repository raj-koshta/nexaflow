    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-1">
                <li class="breadcrumb-item"><a href="{{ route('leads.index') }}" class="text-decoration-none">Leads</a></li>
                <li class="breadcrumb-item active" aria-current="page">{{ Str::limit($lead->name, 20) }}</li>
            </ol>
        </nav>
        <h1 class="h2 fw-bold mb-0">Lead Profile</h1>
    </div>
    <div class="btn-toolbar mb-2 mb-md-0">
        <button type="button" class="btn btn-outline-secondary shadow-sm me-2" onclick="location.href='{{ route('leads.index') }}'">
            ...
            <div class="card-body">
                <div class="d-flex align-items-center mb-4">
                    ...
                    <div>
                        ...
                        @php
                            $sColor = match($lead->status) {
                                'new' => 'info', 'contacted' => 'primary', 'qualified' => 'warning', 'lost' => 'secondary', 'won' => 'success', default => 'secondary'
                            };
                        @endphp
                        <span class="badge bg-{{ $sColor }} bg-opacity-10 text-{{ $sColor }} border border-{{ $sColor }} border-opacity-25 rounded-pill text-capitalize"><i class="bi bi-record-circle me-1"></i>{{ $lead->status }}</span>
                    </div>
                </div>

                <div class="mb-3">
                    <small class="text-muted d-block mb-1">Company</small>
                    <span class="fw-medium text-main">{{ $lead->company ?: 'N/A' }}</span>
                </div>

                <div class="mb-3">
                    <small class="text-muted d-block mb-1">Email</small>
                    <a href="mailto:{{ $lead->email }}" class="text-decoration-none fw-medium text-main"><i class="bi bi-envelope me-2 text-muted"></i>{{ $lead->email }}</a>
                </div>
                
                <div class="mb-3">
                    <small class="text-muted d-block mb-1">Phone</small>
                    <a href="tel:{{ $lead->phone }}" class="text-decoration-none fw-medium text-main"><i class="bi bi-telephone me-2 text-muted"></i>{{ $lead->phone ?: 'N/A' }}</a>
                </div>
                
                <div class="mb-3">
                    <small class="text-muted d-block mb-1">Source</small>
                    <span class="fw-medium text-main text-capitalize">{{ str_replace('_', ' ', $lead->source) }}</span>
                </div>

                <div class="mb-3">
                    <small class="text-muted d-block mb-1">Expected Value</small>
                    <span class="fw-bold text-success">${{ number_format($lead->expected_value, 2) }}</span>
                </div>
                ...
                <div class="mb-0">
                    ...
                    <span class="fw-medium text-main">
                        @if($lead->assignee)
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($lead->assignee->name) }}&background=8b5cf6&color=fff&size=24" class="rounded-circle me-1" alt="{{ $lead->assignee->name }}">
                            {{ $lead->assignee->name }}
                        @else
                            <i class="bi bi-person-x me-1 text-muted"></i> Unassigned
                        @endif
                    </span>
                </div>
            </div>
    ...
    <div class="col-lg-8">
        ...
        <div class="tab-content" id="profileTabsContent">
                            ...
                            <div class="timeline-container">
                                @foreach($lead->activities as $activity)
                                    ...
                                    <div class="position-relative mb-4 ps-4">
                                        ...
                                        <small class="text-muted">Logged by {{ $activity->creator->name ?? 'System' }}</small>
                                    </div>
                                @endforeach
                            </div>
                        @else
                            <div class="text-center py-5 text-muted">
                                ...
                                <p class="mb-0">No activities recorded for this lead yet.</p>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="tab-pane fade" id="documents" role="tabpanel" aria-labelledby="documents-tab">
                @include('leads.partials.documents')
            </div>
        </div>
    </div>
</div>
